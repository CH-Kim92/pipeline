...
function MyAccountPage(){
  ...
  useEffect(()=> {
    axios.get(`http://127.0.0.1:8000/housedata/uid/${user.user_id}`)
    .then((res)=>{
      console.log(res)
      sethouses(res.data)
      setIsLoading(false);
    })
    axios.get(`http://127.0.0.1:8000/request/userID/${user.user_id}`)
    .then((res)=>{
      console.log(res)
  ...
  const houseDelete = (houseid) => {
    axios.delete(`http://127.0.0.1:8000/housedata/${houseid}`).then(
    navigate(`/`)
    )
  }
  ...
  const deleteRequest = (requestid) => {
    axios.delete(`http://127.0.0.1:8000/request/${requestid}`)
    navigate(`/`)
  }
    ...
    {houses.map((house) => (
      ...
      <div key={house.id} className={classes.item}>
        ...
        <div className={classes.content}>
          ...
          <p className={classes.req}>{house.description}</p>
        </div>
